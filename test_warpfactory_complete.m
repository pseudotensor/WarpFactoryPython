...
try
    ...
    nec = ec_module.get_energy_conditions(energy_tensor, metric_shell, 'Null');
    nec_vals = py.numpy.array(nec.values);
    ...
    wec = ec_module.get_energy_conditions(energy_tensor, metric_shell, 'Weak');
    wec_vals = py.numpy.array(wec.values);
    ...
    sec = ec_module.get_energy_conditions(energy_tensor, metric_shell, 'Strong');
    sec_vals = py.numpy.array(sec.values);
    ...
    dec = ec_module.get_energy_conditions(energy_tensor, metric_shell, 'Dominant');
    dec_vals = py.numpy.array(dec.values);
...
try
    ...
    g00_matlab = double(py.array.array('d', py.numpy.nditer(g00_data)));
    T00_matlab = double(py.array.array('d', py.numpy.nditer(T00_data)));
    nec_matlab = double(py.array.array('d', py.numpy.nditer(nec_vals)));
    wec_matlab = double(py.array.array('d', py.numpy.nditer(wec_vals)));
    sec_matlab = double(py.array.array('d', py.numpy.nditer(sec_vals)));
    dec_matlab = double(py.array.array('d', py.numpy.nditer(dec_vals)));
    ...
    disp('   Variables: g00_matlab, T00_matlab, nec_matlab, wec_matlab, sec_matlab, dec_matlab');
...
disp('5. Stress-energy tensor computed');
